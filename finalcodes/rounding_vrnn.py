...
def compute_rnn_grads(loss,network):
	...
	for layer in layers:
		...
		if params:
			for param in params:
				print(param.name)
				if param.name == 'W_in_to_hid':
					grads.append(theano.grad(loss, wrt=layer.W_in_to_hid_d))
				elif param.name == 'W_hid_to_hid':
					grads.append(theano.grad(loss, wrt=layer.W_hid_to_hid_d))
				elif param.name == 'b_hid':
					grads.append(theano.grad(loss, wrt=layer.b_d))
